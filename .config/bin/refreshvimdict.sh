...
if [[ ! -f "./standard_objects.txt" ]]; then
	...
	if [[ -f "./standard_objects_extended.txt" ]]; then
		while read objectname; do
			sf data query -q "select name,entitydefinitionid from entityparticle where entitydefinitionid='$objectname'" -t -r csv >>schema.txt
...
# Get custom fields
sf data query -q "Select developername,TableEnumOrId from customfield" -t -r csv >>schema.txt
